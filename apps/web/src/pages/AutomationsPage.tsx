...
function RuleEditor({
  ...
  const save = async () => {
    ...
    if (actions.length === 0) {
      setError(t('automations.actionRequired'));
      return;
    }
    setSaving(true);
    setError(null);
    ...
    try {
      ...
      setError(describeError(err, t('automations.saveError')));
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async () => {
    ...
    try {
      ...
      toast.error(describeError(err, t('automations.deleteError')));
    }
  };
  ...
  return (
    <Slideover
      open
      onClose={onClose}
      title={editing ? t('automations.editTitle') : t('automations.newTitle')}
      footer={
    ...
    >
      <div className="space-y-5">
        <div className="space-y-2">
          <Label htmlFor="auto-name">{t('automations.nameLabel')}</Label>
          <Input
            id="auto-name"
            value={name}
        ...
        <div className="space-y-2">
          <Label htmlFor="auto-when">{t('automations.whenLabel')}</Label>
          <select
            id="auto-when"
          ...
          {(triggerType === 'device-online' || triggerType === 'device-offline') && (
            <select
              aria-label={t('automations.trigger.networkDevice')}
              className={SELECT_CLASS}
              value={triggerMac}
          ...
          {(triggerType === 'iot-on' || triggerType === 'iot-off') && (
            <select
              aria-label={t('automations.trigger.iotDevice')}
              className={SELECT_CLASS}
              value={triggerDevice}
          ...
          {triggerType === 'sensor-threshold' && (
            <div className="flex flex-wrap items-center gap-2">
              <select
                aria-label={t('automations.trigger.sensor')}
              ...
              <select
                aria-label={t('automations.trigger.comparison')}
                className={cn(SELECT_CLASS, 'w-auto')}
                value={sensorOp}
                onChange={(e) => setSensorOp(e.target.value as 'gt' | 'lt')}
              >
                <option value="gt">{t('automations.trigger.opGt')}</option>
                <option value="lt">{t('automations.trigger.opLt')}</option>
              </select>
              <Input
                type="number"
                aria-label={t('automations.trigger.threshold')}
                value={sensorValue}
          ...
          {triggerType === 'time' && (
            <div className="space-y-2">
              {dayPicker(timeDays, (d) => setTimeDays((prev) => toggleIn(prev, d)), t('automations.trigger.triggerDays'))}
              <Input
                type="time"
                aria-label={t('automations.trigger.triggerTime')}
                value={timeAt}
          ...
          {(triggerType === 'person-arrived' || triggerType === 'person-left') && (
            <select
              aria-label={t('automations.trigger.person')}
              className={SELECT_CLASS}
              value={triggerUserId}
              onChange={(e) => setTriggerUserId(e.target.value)}
            >
              <option value="">{t('automations.trigger.anyPerson')}</option>
              {users.map((u) => (
          ...
          {triggerType === 'mode-changed' && (
            <select
              aria-label={t('automations.trigger.homeMode')}
              className={SELECT_CLASS}
              value={triggerMode}
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-kr-sm text-kr-primary">
            ...
            />
            {t('automations.condition.windowToggle')}
          </label>
          {withWindow && (
            <div className="space-y-2 rounded-lg border border-kr bg-kr-elevated p-3">
              {dayPicker(windowDays, (d) => setWindowDays((prev) => toggleIn(prev, d)), t('automations.condition.windowDays'))}
              <div className="flex items-center gap-2">
                <Input
                  type="time"
                  aria-label={t('automations.condition.from')}
                  value={windowFrom}
                  onChange={(e) => setWindowFrom(e.target.value)}
                />
                <span className="text-kr-xs text-kr-muted">{t('automations.condition.rangeSep')}</span>
                <Input
                  type="time"
                  aria-label={t('automations.condition.to')}
                  value={windowTo}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <Label>{t('automations.action.thenLabel')}</Label>
            <Button
              variant="outline"
              size="sm"
              type="button"
          ...
          <ul className="space-y-2">
            {actions.map((row, i) => (
              <li key={i} className="space-y-2 rounded-lg border border-kr bg-kr-elevated p-3">
                <div className="flex items-center gap-2">
                  <select
                    aria-label={t('automations.action.ariaAction', { n: i + 1 })}
                    className={SELECT_CLASS}
                    value={row.type}
                  ...
                  {actions.length > 1 && (
                    <Button
                      ...
                      size="sm"
                      type="button"
                      aria-label={t('automations.action.remove', { n: i + 1 })}
                ...
                {row.type === 'iot-set' && (
                  <div className="flex items-center gap-2">
                    <select
                      aria-label={t('automations.action.ariaDevice', { n: i + 1 })}
                      className={SELECT_CLASS}
                      value={row.deviceId}
                    ...
                    <select
                      aria-label={t('automations.action.ariaState', { n: i + 1 })}
                      className={cn(SELECT_CLASS, 'w-auto')}
                      value={row.on ? 'on' : 'off'}
                      onChange={(e) => updateAction(i, { on: e.target.value === 'on' })}
                    >
                      <option value="on">{t('automations.action.turnOn')}</option>
                      <option value="off">{t('automations.action.turnOff')}</option>
                    </select>
                  </div>
                )}
                {row.type === 'scene-run' && (
                  <select
                    aria-label={t('automations.action.ariaScene', { n: i + 1 })}
                    className={SELECT_CLASS}
                    value={row.sceneId}
                ...
                {(row.type === 'device-block' || row.type === 'device-pause') && (
                  <div className="space-y-2">
                    <select
                      aria-label={t('automations.action.ariaTarget', { n: i + 1 })}
                      className={SELECT_CLASS}
                      value={row.mac}
                      onChange={(e) => updateAction(i, { mac: e.target.value })}
                    >
                      {eventHasMac && <option value="">{t('automations.action.eventDevice')}</option>}
                      {networkDevices.map((d) => (
                        <option key={d.mac} value={d.mac}>
                          {networkLabel(d)}
                        </option>
                    ...
                    {row.type === 'device-pause' && (
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          min={1}
                          max={1440}
                          aria-label={t('automations.action.ariaPauseMinutes', { n: i + 1 })}
                          value={row.minutes}
                          onChange={(e) => updateAction(i, { minutes: Number(e.target.value) })}
                          className="w-24"
                        />
                        <span className="text-kr-xs text-kr-muted">{t('automations.builder.minutes')}</span>
                      </div>
                    )}
                  </div>
                )}
                {row.type === 'notify' && (
                  <Input
                    aria-label={t('automations.action.ariaMessage', { n: i + 1 })}
                    value={row.message}
                    maxLength={200}
                    placeholder={t('automations.action.messagePlaceholder')}
        ...
        <div className="flex items-center gap-2">
          <Label htmlFor="auto-cooldown" className="text-kr-sm text-kr-secondary">
            {t('automations.builder.cooldownLabel')}
          </Label>
          <Input
            id="auto-cooldown"
            type="number"
    ...
    <div className="space-y-6 p-6">
      ...
      {rules === null ? (
        ...
        !error && (
          <div className="flex flex-col items-center gap-3 rounded-xl border border-kr bg-kr-surface py-12 text-center">
            <p className="text-kr-secondary">{t('automations.empty')}</p>
            <p className="mx-auto max-w-md text-kr-sm text-kr-muted">{t('automations.examples')}</p>
            {isAdmin && (
              <Button
                onClick={() => {
                  setEditing(null);
              ...
              >
                ...
                >
                  ...
                  <span className="block truncate text-kr-sm text-kr-muted">
                    {t('automations.summaryPhrase', {
                      trigger: describeTrigger(rule.trigger, ctx),
                      condition: condition ? ` (${condition})` : '',
                      actions: rule.actions.map((a) => describeAction(a, ctx)).join(' · '),
                    })}
                  </span>
                </button>
                {isAdmin && (
                  <Switch
                    ...
                    aria-label={t('automations.ariaEnable', { name: rule.name })}
                  />
                )}
              </li>
            );
      ...
      {runs.length > 0 && (
        <section className="space-y-2 border-t border-kr-muted pt-6">
          <h3 className="text-kr-lg font-semibold text-kr-primary">{t('automations.runsTitle')}</h3>
