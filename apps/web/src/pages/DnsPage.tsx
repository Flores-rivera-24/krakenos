    ...
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          ...
          >
            {t('dns.domainBlocking')}
          </GlossaryTerm>{' '}
          {t('dns.subtitleSuffix')}
        </p>
      </div>
      ...
      {isAdmin && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base text-foreground">{t('dns.blockDomain')}</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-base text-foreground">{t('dns.blocklist')}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto rounded-md border border-border">
              <table className="w-full text-sm">
                <thead className="bg-secondary text-secondary-foreground">
                  <tr>
                    <th className="px-3 py-2 text-left">{t('dns.domain')}</th>
                    {isAdmin && <th className="px-3 py-2 text-right">{t('dns.action')}</th>}
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={isAdmin ? 2 : 1} className="px-3 py-8 text-center">
                        <p className="text-kr-muted">{t('dns.empty')}</p>
                        <p className="mx-auto mt-1 max-w-xs text-kr-xs text-kr-secondary">
                          {t('dns.emptyHint')}{' '}
                          {isAdmin && t('dns.emptyHintAdmin')}
                        </p>
                      </td>
                    </tr>
                  ) : (
                        ...
                        {isAdmin && (
                          <td className="px-3 py-2 text-right">
                            <DeleteButton
                              ...
                              aria-label={t('dns.removeAria', { domain: b.domain })}
                            >
                              {t('dns.remove')}
                            </DeleteButton>
                          </td>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base text-foreground">{t('dns.recentQueries')}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto rounded-md border border-border">
              <table className="w-full text-sm">
                <thead className="bg-secondary text-secondary-foreground">
                  <tr>
                    <th className="px-3 py-2 text-left">{t('dns.domain')}</th>
                    <th className="px-3 py-2 text-left">{t('dns.client')}</th>
                    <th className="px-3 py-2 text-left">{t('dns.status')}</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                      ...
                      <tr key={`${q.timestamp}-${i}`} className="border-t border-border">
                        ...
                        <td className="px-3 py-2">
                          <span className={q.blocked ? 'text-destructive' : 'text-green-500'}>
                            {q.blocked ? t('dns.blockedStatus') : t('dns.allowedStatus')}
                          </span>
                        </td>
                      </tr>
                    ))
