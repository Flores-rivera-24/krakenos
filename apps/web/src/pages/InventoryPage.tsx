...
const FILTERS: { value: ActiveFilter; key: TranslationKey }[] = [
  { value: 'online', key: 'inventory.filter.online' },
  { value: 'offline', key: 'inventory.filter.offline' },
  { value: 'blocked', key: 'inventory.filter.blocked' },
...
function statusOf(d: Device): {
  ...
  if (d.isBlocked) return { dot: 'danger', text: 'inventory.status.blocked', danger: true };
  return d.online
    ? { dot: 'online', text: 'inventory.status.online', danger: false }
    : { dot: 'offline', text: 'inventory.status.offline', danger: false };
}
          ...
          <tr>
            ...
            {!simpleMode && (
              <>
                ...
                <th scope="col" className="px-3 py-2 text-left font-medium">
                  {t('inventory.col.vendor')}
                </th>
              </>
            )}
            <th scope="col" className="px-3 py-2 text-left font-medium">
              {t('inventory.col.type')}
            </th>
            <th scope="col" className="px-3 py-2 text-left font-medium">
              {t('inventory.col.status')}
            </th>
            <th scope="col" className="px-3 py-2 text-right font-medium">
              {t('inventory.col.lastSeen')}
            </th>
          </tr>
        </thead>
        <tbody>
    ...
    <div className="space-y-4 p-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="inline-flex rounded-md border border-kr bg-kr-elevated p-0.5">
          {(['all', 'groups'] as const).map((tabKey) => (
            <button
              key={tabKey}
              type="button"
            ...
            >
              {tabKey === 'all' ? t('inventory.tab.all') : t('inventory.tab.groups')}
            </button>
          ))}
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-xl border border-kr bg-kr-surface py-16 text-center">
          ...
          <p className="mx-auto max-w-md text-kr-sm text-kr-muted">{t('inventory.empty.desc')}</p>
