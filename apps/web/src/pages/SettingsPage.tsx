        ...
        <div className="space-y-6">
          ...
          {section === 'sistema' && (
            <>
              <Card>
                <CardHeader>
                  <CardTitle>{t('settings.section.system')}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <Setting label={t('settings.system.scanInterval')}>
                    ...
                    >
                      <option value="30">{t('settings.dur.30sec')}</option>
                      <option value="60">{t('settings.dur.1min')}</option>
                      <option value="300">{t('settings.dur.5min')}</option>
                    </select>
                  </Setting>
                  <Setting label={t('settings.system.presenceGrace')}>
                    ...
                    >
                      <option value="5">{t('settings.dur.5min')}</option>
                      <option value="10">{t('settings.dur.10min')}</option>
                      <option value="20">{t('settings.dur.20min')}</option>
                      <option value="30">{t('settings.dur.30min')}</option>
                    </select>
                  </Setting>
                  <Setting label="HTTPS">
          ...
          {section === 'red' && (
            <>
              <Card>
                <CardHeader>
                  <CardTitle>{t('settings.network.driverActive')}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex items-center justify-between text-kr-base">
                    <span className="text-kr-secondary">{t('settings.network.driver')}</span>
                    <span className="font-mono text-kr-primary">{data?.info.driver ?? '—'}</span>
                  </div>
                  <div className="flex items-center justify-between text-kr-base">
                    <span className="text-kr-secondary">{t('settings.network.host')}</span>
                  ...
                  {isAdmin && (
                    <div className="flex items-center gap-3">
                      <Button
                        ...
                        disabled={testing}
                      >
                        {testing ? t('settings.network.testing') : t('settings.network.testConn')}
                      </Button>
                      {test && (
                        <span className="flex items-center gap-2 text-kr-sm">
                          <StatusDot status={test.ok ? 'online' : 'danger'} />
                          {test.ok
                            ? t('settings.network.testOk', { ms: test.latencyMs ?? 0 })
                            : test.error}
                        </span>
                      )}
                    </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle>{t('settings.network.retention')}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <Setting label={t('settings.network.trafficHistory')}>
                    <select
                      className={SELECT_CLASS}
                      aria-label={t('settings.network.trafficHistoryAria')}
                      value={setting('trafficRetentionDays')}
                      disabled={!isAdmin}
                      onChange={(e) => void patch('trafficRetentionDays', e.target.value)}
                    >
                      <option value="7">{t('settings.dur.7days')}</option>
                      <option value="30">{t('settings.dur.30days')}</option>
                      <option value="90">{t('settings.dur.90days')}</option>
                    </select>
                  </Setting>
                  <Setting label={t('settings.network.auditLog')}>
                    <select
                      className={SELECT_CLASS}
                      aria-label={t('settings.network.auditLogAria')}
                    ...
                    >
                      <option value="30">{t('settings.dur.30days')}</option>
                      <option value="90">{t('settings.dur.90days')}</option>
                      <option value="180">{t('settings.dur.180days')}</option>
                    </select>
                  </Setting>
                </CardContent>
              </Card>
          ...
          {section === 'cuenta' && (
            <>
              ...
              {isAdmin && (
                <Card>
                  <CardHeader>
                    <CardTitle>{t('settings.network.auditLog')}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    {audit === null ? (
                      <LoadingLine />
                          ...
                          <thead className="bg-kr-elevated text-kr-secondary">
                            <tr>
                              <th className="px-3 py-2 text-left">
                                {t('settings.account.colAction')}
                              </th>
                              <th className="px-3 py-2 text-left">IP</th>
                              <th className="px-3 py-2 text-left">
                                {t('settings.account.colWhen')}
                              </th>
                            </tr>
                          </thead>
                          <tbody>
