...
export function VpnPage() {
  ...
  const addPeer = async (e: FormEvent) => {
    ...
    try {
      ...
      setName('');
      toast.success(t('vpn.added'));
      void load();
    } catch (err) {
      toast.error(describeError(err, t('vpn.addError')));
    } finally {
      setBusy(false);
    }
  };
  ...
  const removePeer = async (id: string) => {
    try {
      ...
      toast.success(t('vpn.removed'));
      void load();
    } catch (err) {
      toast.error(describeError(err, t('vpn.removeError')));
    }
  };
  ...
  return (
    <div className="space-y-6 p-6">
            ...
            {loading ? (
              ...
              <>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t('vpn.statusLabel')}</span>
                  <span className={status.enabled ? 'text-green-500' : 'text-muted-foreground'}>
                    {status.enabled ? t('vpn.active') : t('vpn.inactive')}
                  </span>
                </div>
                <div className="flex justify-between">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <span className="flex items-center gap-1.5">
              ...
              <HelpHint content={t('vpn.qrHelp')} label={t('vpn.qrHelpLabel')} />
            </span>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base text-foreground">{t('vpn.authorizedDevices')}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto rounded-md border border-border">
            <table className="w-full text-sm">
              <thead className="bg-secondary text-secondary-foreground">
                <tr>
                  <th className="px-3 py-2 text-left">{t('vpn.name')}</th>
                  <th className="px-3 py-2 text-left">{t('vpn.vpnIp')}</th>
                  <th className="px-3 py-2 text-left">{t('vpn.publicKey')}</th>
                  <th className="px-3 py-2 text-right">{t('vpn.action')}</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={4} className="px-3 py-8 text-center">
                      <p className="text-kr-muted">{t('vpn.empty')}</p>
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-right">
                        <DeleteButton
                          ...
                          aria-label={t('vpn.deleteAria', { name: p.name })}
                        >
                          {t('common.delete')}
                        </DeleteButton>
                      </td>
