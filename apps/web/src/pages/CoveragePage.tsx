...
const VIEWS: { id: CoverageView; labelKey: TranslationKey }[] = [
  { id: 'edit', labelKey: 'coverage.view.edit' },
  { id: 'predict', labelKey: 'coverage.view.predict' },
  { id: 'survey', labelKey: 'coverage.view.survey' },
];
...
export function CoveragePage() {
  ...
  useEffect(() => {
    ...
    void listFloorPlans()
      ...
      .catch((err) => active && setError(describeError(err, t('coverage.loadPlansError'))))
  ...
  const handlePlacePoint = useCallback(
    async (x: number, y: number) => {
      ...
      try {
        ...
        if (!result.found) {
          toast.info(t('coverage.survey.notDetectedHere'));
        } else {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setDirty(false);
      toast.success(t('coverage.planSaved'));
    } catch (err) {
    ...
    <div className="space-y-4 p-6">
      ...
      {!error && plans.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-xl border border-kr bg-kr-surface py-16 text-center">
          ...
          <p className="mx-auto max-w-md text-kr-sm text-kr-muted">{t('coverage.empty.body')}</p>
          {isAdmin && (
          ...
          <>
            ...
            <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
              ...
              <aside className="space-y-4">
                {view === 'edit' && (
                  <>
                    <div className="rounded-xl border border-kr bg-kr-surface p-4 text-kr-sm text-kr-secondary">
                      <p className="mb-2 font-medium text-kr-primary">{t('coverage.editPanel.title')}</p>
                      <ul className="list-disc space-y-1 pl-4 text-kr-muted">
                        <li>{t('coverage.editPanel.wall')}</li>
                        <li>{t('coverage.editPanel.ap')}</li>
                        <li>{t('coverage.editPanel.select')}</li>
                      </ul>
                    </div>
                    <ApPalette
                      placedAps={editAps}
                ...
                {view === 'predict' && (
                  <div className="rounded-xl border border-kr bg-kr-surface p-4 text-kr-sm text-kr-secondary">
                    <p className="font-medium text-kr-primary">{t('coverage.predictPanel.title')}</p>
                    <p className="mt-1 text-kr-muted">{t('coverage.predictPanel.body', { band })}</p>
                  </div>
                )}
              </aside>
            </div>
