            ...
            <span className="min-w-0">
              ...
              <span className="block text-kr-xs text-kr-muted">
                ...
                {plural(room.deviceCount, {
                  one: t('rooms.deviceOne'),
                  other: t('rooms.deviceOther'),
                })}{' '}
                · {room.iotCount} IoT
                {room.anyUnreachable && (
                  <span className="text-warning"> · {t('rooms.noSignal')}</span>
                )}
              </span>
            </span>
          </button>
...
function RoomEditor({
  ...
  return (
    <Slideover
      open
      onClose={onClose}
      title={editing ? t('rooms.edit') : t('rooms.new')}
      footer={
    ...
    >
      <div className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="room-name">{t('rooms.nameLabel')}</Label>
          <Input
            id="room-name"
            value={name}
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-start justify-between gap-3">
        ...
        {isAdmin && (
          <Button onClick={() => setEditor('new')}>{t('rooms.new')}</Button>
        )}
      </div>
      ...
      {rooms === null ? (
        ...
        !error && (
          <div className="flex flex-col items-center gap-3 rounded-xl border border-kr bg-kr-surface py-16 text-center">
            <p className="text-kr-secondary">{t('rooms.empty')}</p>
            <p className="mx-auto max-w-md text-kr-sm text-kr-muted">{t('rooms.emptyHelp')}</p>
            {isAdmin ? (
              <Button onClick={() => setEditor('new')}>{t('rooms.createFirst')}</Button>
            ) : (
