      ...
      <CardContent className="space-y-3">
        ...
        {loading ? (
          ...
          <p className="text-kr-sm text-kr-muted">
            {t('settings.update.disabledPre')}{' '}
            <code className="font-mono text-kr-secondary">UPDATE_CHECK_REPO</code>{' '}
            {t('settings.update.disabledPost')}
          </p>
          ...
          <div className="flex items-center gap-2 rounded-lg border border-warning/40 bg-warning/10 px-3 py-2 text-kr-sm text-warning">
            ...
            <span>
              {t('settings.update.available')} <span className="font-mono">{status.latest}</span>
            </span>
          </div>
        ) : (
