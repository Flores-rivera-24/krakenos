  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle>{t('dashboard.widget.traffic.title')}</CardTitle>
        {last && (
      ...
      <CardContent>
        {data.length < 2 ? (
          <p className="py-12 text-center text-kr-sm text-kr-muted">
            {t('dashboard.widget.traffic.waiting')}
          </p>
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={data}>
              ...
              <Area
                type="monotone"
                dataKey="rx"
                name={t('dashboard.widget.traffic.download')}
              ...
              <Area
                type="monotone"
                dataKey="tx"
                name={t('dashboard.widget.traffic.upload')}
                stroke="var(--kr-success)"
                fill="url(#tx)"
              />
            </AreaChart>
          </ResponsiveContainer>
