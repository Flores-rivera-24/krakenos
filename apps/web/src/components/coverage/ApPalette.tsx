...
export function ApPalette({
  ...
  useEffect(() => {
    ...
    void listPlaceableAccessPoints()
      ...
      .catch(
        (err) =>
          active && setError(describeError(err, t('coverage.palette.loadError'))),
      )
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-kr bg-kr-surface p-4">
        <p className="mb-2 text-kr-sm font-medium text-kr-primary">{t('coverage.palette.title')}</p>
        <p className="mb-3 text-kr-xs text-kr-muted">{t('coverage.palette.subtitle')}</p>
        ...
        {loading ? (
          ...
          <p className="text-kr-sm text-kr-muted">{t('coverage.palette.empty')}</p>
        ) : (
      ...
      {placedAps.length > 0 && (
        <div className="rounded-xl border border-kr bg-kr-surface p-4">
          <p className="mb-3 text-kr-sm font-medium text-kr-primary">
            {t('coverage.palette.placedCount', { count: placedAps.length })}
          </p>
              ...
              <li key={ap.id} className="rounded-md border border-kr bg-kr-bg p-3">
                <div className="mb-2 flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    aria-label={t('coverage.palette.remove', { name: ap.name })}
                ...
                <label className="mb-2 block text-kr-xs text-kr-secondary">
                  <span className="mb-1 flex items-center justify-between">
                    <span>{t('coverage.palette.power')}</span>
                    <span className="tabular-nums text-kr-muted">
                      {t('coverage.palette.powerValue', { value: ap.txPowerDbm })}
                    </span>
                  </span>
                  <input
                    type="range"
                    ...
                    aria-label={t('coverage.palette.powerAria', { name: ap.name })}
                  />
                </label>

                <div className="mb-2">
                  <span className="mb-1 block text-kr-xs text-kr-secondary">{t('coverage.palette.bands')}</span>
                ...
                <label className="flex items-center justify-between text-kr-xs text-kr-secondary">
                  <span>{t('coverage.palette.active')}</span>
                  <Switch
                    checked={ap.enabled}
                    disabled={!canEdit}
                    onCheckedChange={(v) => onUpdateAp(ap.id, { enabled: v })}
                    aria-label={t('coverage.palette.activate', { name: ap.name })}
                  />
                </label>
              </li>
            ))}
