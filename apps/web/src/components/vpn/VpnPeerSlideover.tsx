  ...
  return (
    ...
    >
      <dl className="space-y-2 rounded-lg border border-kr bg-kr-elevated p-3">
        <Row label={t('vpn.vpnIp')} value={peer.allowedIps} />
        <Row label={t('vpn.publicKey')} value={peer.publicKey} />
        <Row label={t('vpn.created')} value={new Date(peer.createdAt).toLocaleString()} />
        <Row
          label={t('vpn.lastHandshake')}
          value={peer.lastHandshake ? new Date(peer.lastHandshake).toLocaleString() : t('vpn.never')}
        />
      </dl>

      {config ? (
        <div className="mt-4 space-y-3">
          <p className="text-kr-sm text-warning">{t('vpn.qrWarning')}</p>
          <img
            src={config.qr}
            alt={t('vpn.qrAlt')}
        ...
        <p className="mt-4 text-kr-xs text-kr-muted">{t('vpn.configOnlyOnCreate')}</p>
      )}
    </Slideover>
  );
}
