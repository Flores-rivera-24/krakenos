...
export const PLUGS_GUIDES_EN: GuideTranslations = {
  ...
  switchbot: {
    ...
    vendor: 'SwitchBot',
    intro:
      'SwitchBot has very versatile devices: from the “Bot” that physically presses a button to plugs and color bulbs. KrakenOS controls them through the SwitchBot Hub Mini or Hub 2, using their local access (without the cloud). You need to enable that access in the app and copy a “token” that acts as a key.',
    prerequisites: [
      'A SwitchBot Hub Mini or Hub 2 (the other hub models do not work here).',
      'The SwitchBot app to enable local control and get the token.',
      'The hub having a fixed IP on your router.',
    ],
    steps: [
      {
        title: 'Enable local control in the app',
        body: 'Open the SwitchBot app, go into your Hub Mini/Hub 2 and enable the “LAN API” or “local control” option (the name varies by version). Make sure the hub has a fixed IP reserved on your router.',
      },
      {
        title: 'Copy the token',
        body: 'In the SwitchBot app, go to “Profile → Preferences → Developer Mode” and copy the “Token”. It is a long key that authorizes access, like a password. Keep it handy for the next step.',
        note: 'Treat the token like a password: do not share it. It is stored encrypted.',
      },
      {
        title: 'Connect KrakenOS',
        body: 'Enter the hub’s address, its port (8123 by default) and the token. Once saved, the compatible devices appear (Bot, Plug Mini, color bulbs, strips…). Sensors and locks are not shown in this version.',
      },
    ],
    fields: {
      hubHost: {
        label: 'Hub address',
        help: 'The Hub Mini/Hub 2’s IP on your network, for example 192.168.1.90. It is best if it is fixed.',
        placeholder: '192.168.1.90',
      },
      hubPort: {
        label: 'Hub port',
        help: 'The “door” the hub answers on. By default it is 8123.',
      },
      token: {
        label: 'Token',
        help: 'The key you copied from the SwitchBot app’s developer mode. It is stored encrypted.',
      },
    },
    troubleshooting: [
      {
        q: 'No device appears.',
        a: 'Confirm that you enabled local control on the hub, that the token is correct and that the hub’s address and port are correct. Remember that you need a Hub Mini or Hub 2.',
      },
      {
        q: 'I don’t see my sensor or my lock.',
