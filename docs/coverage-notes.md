...
**El agente mide con `all: true`:** cuenta **todo** `src/**`, no solo lo
...
**La web también mide con `all: true`.** Era deuda declarada… hasta que
...
Medido el 2026-07-29 (tras): agente **2291 tests** (240 ficheros), web **631
...
proceso): `src/index.ts`, `src/update-runner.ts` y `src/reset-admin.ts`.
...
- **web** (`all: true`): stmts 85 · branches 78 · funcs **64** · lines 85
...
ausentes (abajo). El suelo de *funciones* de la web es más bajo porque
...
solo suben cuando la verificación con hardware aporte datos reales o tests de contrato adicionales;
no forzar test-por-cobertura sobre ellos.

## Zonas frías — por categoría

### 1. No testeables aquí: hardware/IO real
...
- **`privileged/runner.ts` · `SudoHelperRunner`** (~70%): invoca `sudo` con el
  helper allowlisted; el camino real exige root y el helper instalado.
...
- **Los 2–3 peores huecos de riesgo al pasar a `all: true`:**
  ...
  actualizador **detached** + `unref`, mock de `child_process`);
  `test-connection.ts::testConnection` (prueba transitoria mock de los 8 dominios
  + dominio desconocido → fallo legible). +12 tests.

### 3. Frías restantes de bajo riesgo (deuda consciente)

- **`socketio.ts` línea del `setInterval`** de re-verificación: la lógica
