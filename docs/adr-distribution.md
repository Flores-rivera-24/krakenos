# ADR — Modelo de distribución y canal de updates (US-217)

- **Estado:** Aceptado (2026-07-16)
- **Contexto de la decisión:** con el instalador de un comando entregado (US-216) y el
  lanzamiento a primeros usuarios en curso (US-218), hay que decidir **el vehículo de
...
- **Decisión en una línea:** **instalador nativo ahora** (ya entregado); el canal de updates
  ...
  HU futura gated al feedback de US-218); imagen de SO y caja de hardware quedan **explícitamente
...
|---|---|---|---|---|
| **Instalador nativo** (US-216) | `curl \| sudo bash` sobre Debian/Ubuntu/Pi: Node pinneado, systemd, idempotente | **Pagado** (~1 script + smoke CI) | Bajo: matriz Debian/Ubuntu/Pi OS × x86-64/ARM64 | ✅ **elegido, entregado** |
...
| **Caja de hardware** | Aparato con KrakenOS preinstalado (el camino validado del nicho: HA Green $99, Firewalla) | Muy alto: hardware, certificaciones, inventario, logística, soporte | Muy alto | ⏸ aplazada — es el **modelo de negocio validado** del sector, no una opción técnica; se reevalúa solo con demanda demostrada (US-218) |
| **Docker** | Imagen todo-en-uno no privilegiada | Pagado | Bajo | ✅ se queda como **demo/evaluación** (US-211): no opera VPN/firewall/QoS/cámaras/descubrimiento (`docker-limitations.md`) |

## Canal de updates: de `git + pnpm` a tarballs con checksum

**Hoy** (US-190): el orquestador hace `git fetch/checkout` + `pnpm install` + build en el
...
US-190 pasa de `fetch+build` a `descargar → verificar checksum → desempaquetar en un
...
directorio anterior). El instalador (US-216) ya está preparado: `--update` delega en el
...
futura que se abrirá cuando US-218 confirme que hay usuarios actualizando de verdad. Hasta
...
financiar el desarrollo). Con US-218 se evalúan, por observación y no por encuesta:
...
2. **¿El acceso remoto es el dolor?** (cuántos usuarios llegan al muro CGNAT y usan
   Tailscale, US-215). Si es mayoría, un **relay propio** estilo Nabu Casa es el candidato de
...
- US-218 muestra **>25% de updates fallidos/revertidos** con el canal git+pnpm → adelantar la
  HU de tarballs.
...
> US-190) · `docs/docker-limitations.md` (por qué Docker es demo) · US-216 (instalador).
